...
for sub in subs:
	for sesh in range(1,3):
		...
		for i,tissuetype in ['csf','gm','wm']:

			tissue_thresh=['0.96','0.7','0.96']
			...
			## TP to Rest
			## Apply XFM
			app_xfm_lin_prob.append(pe.Node(interface=fsl.ApplyXfm(),name='app_xfm_lin_prob_'+tissuetype))
			app_xfm_lin_prob[i].inputs.apply_xfm = True
			app_xfm_lin_prob[i].inputs.in_file = prob[i]

			## PVE to Rest
			## Apply XFM
			app_xfm_lin_pve.append(pe.Node(interface=fsl.ApplyXfm(),name='app_xfm_lin_pve_'+tissuetype))
			app_xfm_lin_pve[i].inputs.apply_xfm = True
			app_xfm_lin_pve[i].inputs.in_file = pve[i]

			## Mask Prob Map
			prob_bin.append(pe.Node(interface=afni.preprocess.Calc(),name='prob_bin_'+tissuetype))
			prob_bin[i].inputs.in_file_a = prob_
			prob_bin[i].inputs.expr = 'ispositive(a-'+tissue_thresh[i]+')'
